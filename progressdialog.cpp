...
void ProgressDialog::execute(Cervisia::CommandBase* cmd)
{
    ...
    connect(d->timer, SIGNAL(timeout()), this, SLOT(timeoutOccurred()));
...
{
    kdDebug() << "ProgressDialog::slotOk()" << endl;
    reject();
}
...
{
    kdDebug() << "ProgressDialog::slotCancel()" << endl;
...
void ProgressDialog::timeoutOccurred()
{
    stopNonGuiPart();
    startGuiPart();
}
